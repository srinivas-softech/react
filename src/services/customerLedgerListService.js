...
export const getAllCustomers = async (onSuccess, onFailure, q) => {
  ...
  try {
    const res = await webApi.post("/reports/customerLedgerList/list", {
    ...
    if (res.status === 200) {
      ...
      r.map((c, i) => {
        allCusts.push({
          id: i + 1,
          customer_id: c.customer_id,
          cusLedCompany: c.company_name,
          cusLedGroup_name: c.group_name,
          cusLedLedger_account_name:c.ledger_account_name,
          group_id: c.group_id,
          reference_id: c.reference_id,
          reference_name: c.reference_name,
          website: c.website,
