...
export const getsearchAllLedgerAccount = async (onSuccess, onFailure ,v) => {
  try {
    ...
    console.log(v.ddl_geter?.value,"value")

    const res = await webApi.post("/master/ledger_account_name/list", {
      ledger_account_id:v.ddl_ledger_account?.value,
      ledger_group_id: v.ddl_ledger_group?.value,
      dr_cr_status: v.ddl_Status?.value,
      closingBalance: v.ddl_geter?.value,


      
    });
    if (res.status === 200) {
      // const r = res.data.filter((e)=>{e.ledgerClosingStatus === v.ddl_Status?.value});
      // const r = res.data.filter((e) => e.ledgerClosingStatus === v?.ddl_Status?.value);
      const r = res.data.filter((e) => {
        if (v?.ddl_Status?.value) {
          return e.closeCrDrStatus === v.ddl_Status.value;
        } else {
          return true;           // return true for every element
        }
      });
      ...
      r.map((r, i) => {
        allLedgerAcc.push({
          id: i + 1,
          ledger_account_id: r.ledger_account_id,
          ledgerAccount: r.ledgerName,
          // vendor_customer:r.vendor_name ? r.vendor_name: r.customer_name,
          vendor_customer: r.vendor_name ?  r.vendor_name : (r.customer_name ? r.customer_name : ""),

          group:r.group_name,
          alias: r.alias,
          ledgerGroup: r.ledger_group_id,
          ledgerGroupName: r.ledger_group_name,
          openingBalance: Number(Math.abs(r.openingBalance).toFixed(2)),
          openCrDrStatus: r.openCrDrStatus,
          closingBalance: Number(Math.abs(r.closingBalance).toFixed(2)),
          closingBalanceView: Math.abs(r.closingBalance).toFixed(2) + " " + r.closeCrDrStatus,
          closing_dr_cr_status:r.closeCrDrStatus,
          openingBalanceView: r.openingBalance + " " + (r.openCrDrStatus ? r.openCrDrStatus:"" ),
          dr_cr_status: r.dr_cr_status,
          creditLimit: r.credit_limit,
          action_items: r.action_items,
          status: r.active_status,
