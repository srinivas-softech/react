  ...
  try {
    ...
    if (res.status === 200) {
      ...
      r.map((r, i) => {
        ...
        // combineCharges = r.sumSalesOrderCharges.reduce((s, l) => s.concat(l))
        combineCharges = r?.sumSalesOrderCharges?.length > 0 ? r?.sumSalesOrderCharges.reduce((s, l) => s.concat(l)) : []
        ...
        allItems.push({
          ...
          company_name: r.company_name,
          closing_balance: currencyFormate(
            r.receipt_payments?.length > 0 ?
              Math.abs(r.closing_balance) + Number(r.sumSalesOrderNetValue) +
              combineCharges.reduce((k, l) =>
                l.charge_type === "+" ?
                  + Number(l.charge_amount) : - Number(l.charge_amount)
                , 0) + Number(r.receipt_payments[0]?.debit_balance) - Number(r.receipt_payments[0]?.credit_balance)
              :
              Math.abs(r.closing_balance) + Number(r.sumSalesOrderNetValue) +
              combineCharges.reduce((k, l) =>
                l.charge_type === "+" ?
...
// console.log(error,"sen1009")

    onFailure("Something Wrong! Please Try again later52 " + error);
  }
};
...
export const getSearchLedgerVouchers = async (onSuccess, onFailure, id,cust_id,from_date,to_date,searchQuery) => {
  // console.log(from_date,to_date,"abc34")
  
  try {

    // console.log(from_date,"sen04022023",to_date)
    ...
    const res = await webApi.post("/reports/AccountVoucherForSalesOrder/list", {
      ...
      voucher_to_date: timestamp(to_date)+ 86399,
    ...
    if (res.status === 200) {
      ...
      // console.log(r,"sen1309")
          ...
          narraction:r.narration,
            ...
            r.sales_status==="37"?r.invoice_other_charges?.length >0 ? r.invoice_other_charges :0
             :
...
export const getLedgerByCustomerName = async (customer_id, onSuccess, onFailure) => {

  // console.log(customer_id, "999d2")
  try {
    const res = await webApi.post("/master/view_ledger_account_byName/list", {
