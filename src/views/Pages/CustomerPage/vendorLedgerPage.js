...
import {
  ...
  //   deleteVendor,
  getAllGroup,
} from "../../../services/VendorLedgerListService";
...
const vendorLedgerPage = () => {
  ...
  const onhandleExportToExcel = async () => {
    ...
    const titleRow = worksheet.addRow(["Report Name : Vendor Ledger List"]);
    ...
    const headers = ["Sl No", "Group Name", "Company", "Ledger Account"];
    ...
    const VendorTable = Vendors.map((Vendor) => {
      return {
        id: Vendor.id,
        group_name: Vendor.LedGroup_name,
        company: Vendor.LedCompany,
        ledger_account: Vendor.LedLedger_account_name,
      };
    });
    ...
    const keyword_pharse = addSearch?.txt_keyword_pharse
      ? addSearch?.txt_keyword_pharse
      : "-";
    ...
    saveAs(data, "Vendor Ledger List .xlsx");
  };

  const onSearchVendor = (e) => {
    setRefresh(false);
    e.preventDefault();
    setLoading(true);
    ...
    getAllVendors(
      (Vendors) => {
        if (Vendors.length) {
          ...
          dispatch({
            type: actionTypes.SET_OPEN_MSG,
            payload: { msg: err, "Vendor not found": "info" },
          });
        }
      },
      (err) => {
  ...
  return (
    <ThemeProvider theme={theme}>
      <PageTitle
        title="Master > Vendor - Ledger List"
        // addBtnLink="/admin/master/Vendor-add"
        // btnToolTip="Add Vendors"
      />
      <>
        <GridContainer className={classes.root}>
          <GridItem xs="12">
            <CustomCard
              cdTitle="Search Vendor -Ledger List"
          ...
          >
            <GridItem xs="12">
              <Card className={classes1.headerCard}>
                ...
                >
                  {!refresh ? (
                    <MuiTable columns={headerData} rows={Vendors} />
                  ) : (
                    ""
                  )}
                </CardBody>
