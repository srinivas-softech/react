...
const BrandWiseReport = () => {
  ...
  const onhandleExportToExcelCondensed = async () => {
    ...
    const titleRow = worksheet.addRow(['Report Name : Brand Wise Report']);
    ...
    const BrandWiseReportTable = condensed.map((acc) => {
      return {
              Sl_No: acc.ids,
        Brand: acc.brand[0],
        Quantity: acc.qty,
    ...
    BrandWiseReportTable.forEach((data) => {
    ...
    periodCell.value = `Brand: ${category}       Period:  ${chosenPeriod} 
    ...
    saveAs(data, 'Brand Wise Report.xlsx');
  };
  ...
  const onhandleExportToExcel = async () => {
    ...
    const BrandWiseReportTable = detailed.map((acc) => {
      return {
        Sl_No: acc.ids,
        Brand: acc.brand,
        Item: acc.item,
  ...
  const onClickPdfDetailed = (e) => {
    ...
    autoTable(doc, {
      ...
      body: BrandWiseReportPdf,
      didDrawCell: (BrandWiseReportPdf) => {
        //console.log(BrandWiseReportPdf.column.index);
      },
    });
    doc.save(`BrandWiseReportReport${currentDate()}.pdf`);
  ...
  return (
    <ThemeProvider theme={theme}>
      <PageHeader title="MIS Report > Brand Wise Report " />
      <GridContainer>
        <GridItem xs="12">
          <CustomCard
            cdTitle="BrandWiseReport"
                    ...
                    >
                      <GridItem>
                        <h4 className={classes1.headerCdTitle}>
                          Brand Wise Report Search Result
                        </h4>
                      </GridItem>
