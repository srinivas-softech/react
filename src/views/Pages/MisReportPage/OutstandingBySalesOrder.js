...
const OutstandingBySalesOrder = () => {
  ...
  const [
    OutstandingBySalesOrderPdf,
    setAllOutstandingBySalesOrderPdf,
  ] = React.useState([]);
  ...
  const searchData = [
    [
      ...
      addSearch?.txt_to_date
        ? `As On Date: ${formatDate(addSearch?.txt_to_date)}`
        : "",
  ...
  const onSearchReport = (e) => {
    ...
    if (
      ...
      !addSearch.ddl_group &&
      !addSearch.ddl_reference
    ) {
      dispatch({
        type: actionTypes.SET_OPEN_MSG,
        payload: {
          msg: "Please select  Reference or Group or Customer ",
          msgType: "error",
        },
      });
      return;
    ...
    getAllOutstandingData(
      (r, pdf) => {
        if (r.length) {
          setOutstanding(r);
          setAllOutstandingBySalesOrderPdf(pdf);
  ...
  const onhandleExportToExcel = async () => {
    if (globalState.user.user_role !== "Admin") {
      alert("You do not have permission to Export to Excel.");
      return;
    }
    ...
    const titleRow = worksheet.addRow([
      "Report Name : Outstanding By Sales Order",
    ]);
    ...
    const OutstandingBySalesOrderTable = outstanding.map((item) => {
      return {
        Sl_No: item.id,
        Client: item.company_name[0],
        Total_Outstanding: item.sales_order_total,
    ...
    OutstandingBySalesOrderTable.forEach((data) => {
    ...
    const Customer = addSearch?.ddl_customer?.label
      ? addSearch?.ddl_customer?.label
      : "-";
    const Group = addSearch?.ddl_group?.label
      ? addSearch?.ddl_group?.label
      : "-";
    const Reference = addSearch?.ddl_reference?.label
      ? addSearch?.ddl_reference?.label
      : "-";
    ...
    saveAs(data, "Outstanding By Sales Order.xlsx");
  };
  ...
  const onClickPdf = (e) => {
    ...
    autoTable(doc, {
      head: headerDataPdf,
      body: OutstandingBySalesOrderPdf,
  ...
  return (
    <ThemeProvider theme={theme}>
      <PageHeader title="MIS Report > Outstanding By Sales Order " />
      <GridContainer>
        <GridItem xs="12">
          <CustomCard
            cdTitle=" Outstanding By Sales Order"
              ...
              <Table className={classes.table} stickyHeader aria-label="sticky table">
                <TableHead>
                  <TableRow>
                    ...
                    <StyledTableCell align="right">Sales Order Net Value</StyledTableCell>
