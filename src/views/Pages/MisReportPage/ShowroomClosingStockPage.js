...
import {
  ...
  getItemByBrand,
} from "../../../services/showroomClosingService";
...
import { getSearchItem } from "services/itemsService";
...
const ShowroomWiseSalePage = () => {
  ...
  const onSelect = (name, v) => {
    ...
    if (name === "ddl_brand") {
      //console.log(name, "checkname", v)
      getItemByBrand(v.value, (r) => {

        //console.log(r,"sankitem")
        setItemList(r)
      }
  ...
  const onSearchShowroomwiseSales = (e) => {
    ...
    if (
      ...
      showroomWiseSalesData(
        (r) => {
          //console.log(r,"salesstok")
          setshowroomWiseSalesData(r)
        ...
        addSearch?.ddl_brand?.value, addSearch?.ddl_showroom_warehouse?.value
      )
    }
  ...
  // };
    ...
    //   const ShowroomWiseSalesTable = itemList.map((item) => {
    //     const rowData = {
    //       ID: item.id,
    ...
    //           const closingStock = showroomwiseData.sort(
    //             (a, b) => a.showroom_warehouse_id - b.showroom_warehouse_id
    //           ).filter((o)=>o.item_id === item.item_id)[i].closingStock;
  
    //           rowData[s.label] = closingStock;
    //           Total += closingStock;
    //         }          
    //       });
    ...
    //   //console.log("sen1904=>>", ShowroomWiseSalesTable);
    ...
    const onhandleExportToExcel = async () => {
      ...
      const titleRow = worksheet.addRow(['Report Name :  Showroom Wise Sales Report']);
      ...
      const headers = [
        ...
        ...showShoroms
          .sort((a, b) => a.value - b.value)
          .map((s) => (s.value && s.value !== "All" ? s.label : null))
          .filter(Boolean),
         'Total' 
      ];
      ...
      ShowroomWiseSalesTable.forEach((data) => {
      ...
      const category = addSearch?.ddl_brand?.label ;
      const showroom_warehouse = addSearch?.ddl_showroom_warehouse?.label ;
  
      periodCell.value = `Brand: ${category}       
      ...
      saveAs(data, 'Showroom Wise Sales Report.xlsx');
    };
                        ...
                        return (
                          <TableRow>
                            ...
                            <StyledTableCell
                                ...
                                className={classes.warehouse}
                              >
                                {showroomwiseData.length>0?(showroomwiseData
                                  .sort(
                                    (a, b) =>
                                      a.showroom_warehouse_id -
                                      b.showroom_warehouse_id
                                  ...
                                  .filter((o) => o.item_id === r.item_id)
                                  .reduce((sum, li) => sum + li.closingStock, 0)).toFixed(2):''}
                              </StyledTableCell>
                          </TableRow>
                        );
                      })
