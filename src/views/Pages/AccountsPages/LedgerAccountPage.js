...
const LedgerAccountPage = () => {
  ...
  const [ledgerAccountSearch, setAllLedgerAccountSearch] = React.useState([]);
  ...
  const [error, setError] = React.useState({
    ...
    ddl_ledger_group: false,
    txt_opening_balance: false,
    ddl_drcr: false,
  });
    ...
    switch (info) {
      case "ddl_ledger_group":
        ...
        if (v.value === 0 || !v.value) {
          setError((prevError) => ({ ...prevError, ddl_ledger_group: true }));
        } else {
          setError((prevError) => ({ ...prevError, ddl_ledger_group: false }));
        }
        break;
      default:
        break;
      case "ddl_drcr":
        ...
        if (v.value === 0 || !v.value) {
          setError((prevError) => ({ ...prevError, ddl_drcr: true }));
        } else {
          setError((prevError) => ({ ...prevError, ddl_drcr: false }));
        }
        break;
...
///ledger acc all
  ...
  const onSubmitModel = (e) => {
    ...
    else if (!addLedgerAccount.ddl_ledger_group) {
      setError({
        ddl_ledger_group: addLedgerAccount.ddl_ledger_group ? false : true,
      });
    } 
    else if (!addLedgerAccount.txt_opening_balance) {
      setError({
        txt_opening_balance: addLedgerAccount.txt_opening_balance ? false : true,
      });
    } 
    else if (!addLedgerAccount.ddl_drcr) {
      setError({
        ddl_drcr: addLedgerAccount.ddl_drcr ? false : true,
      });
    } 
    else {
  ...
  const onEditLedgerAccount = (row) => {
    ...
    if(row.ledgerGroup == 54 || row.ledgerGroup == 56){
  ...
  const onSearchLedgerAccountPage = (e) => {
    ...
    getsearchAllLedgerAccount
      (
        (enquery) => {
          if (enquery.length) {
            setAllLedgerAccountSearch(enquery);
            setViewRes("visible");
            setLoading(false);
          } else {
            setViewRes("visible");
  ...
  const onhandleExportToExcel = async () => {
    ...
    const titleRow = worksheet.addRow(['Report Name : ledger Account List']);
    ...
    const ledgerAccountTable = ledgerAccountSearch.map((ledgerAccount) => {
      return {
        id: ledgerAccount.id,
...
if( addSearch?.ddl_ledger_account && addSearch?.ddl_ledger_group){
  periodCell.value = `Ledger Group: ${ledger_group}   Ledger Account: ${ledger_account} `;
    ...
    saveAs(data, 'ledger Account.xlsx');
  }; 
                            ...
                            {item.html_element === "TextField" && (
                              <>
                                ...
                                <TextField
                                  ...
                                  value={addLedgerAccount[item.name]}
                                  variant="outlined"
                                  disabled={item.label=='Ledger Account'? textFieldDisabled : ''}
                                />
                              </>
                            )}
          ...
          {loading ? (
            ...
            <CardHeader className={classes1.TbheaderCdhd} style={{ height: 60 }}>
                <GridContainer justifyContent="space-between" alignItems="center">
                  <GridItem>
                    <h4 className={classes1.headerCdTitle}>ledger Account</h4>
                  </GridItem>
                  {globalState.user.user_role !== "Admin" ? ( 
                  <GridItem style={{ cursor: "pointer",display: "none" }}>
                    <IconButton variant="text" onClick={() => onhandleExportToExcel(ledgerAccountSearch)} >
