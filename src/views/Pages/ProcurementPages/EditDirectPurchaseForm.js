...
const DirectPurchaseForm = () => {
  ...
  const [serching, setSerching] = React.useState(false);
  ...
  React.useEffect(() => {
    if (addedItems.length) {
      setBillDetail((prv) => ({
        ...prv,
        [`txt_bill_value`]: addedItems.reduce(
  ...
  const onChange = (e, row) => {
    ...
    if (name === `chk_gst${row.item_id}`) {
      if (e.target.checked) {
        ...
        const disc_percentage = Number(
          itemDetail[`txt_disc_per${row.item_id}`]
            ? itemDetail[`txt_disc_per${row.item_id}`]
            : 0
        );

        let discValue = itemDetail[`txt_disc_value${row.item_id}`]
          ? itemDetail[`txt_disc_value${row.item_id}`]
          : calculatePerValue(disc_percentage, rate);
        ...
        const gst_amount = Number(
          Number(afterDiscRate) * Number(qty) * (Number(gst_rate) / 100)
        ).toFixed(2);
        // const netValue = Number( afterDiscRate + gst_amount );
        const netValue =
          Number(Number(qty) * (rate - discValue)) + Number(gst_amount);
    ...
    if (name === `txt_disc_per${row.item_id}`) {
      ...
      if (!value) {
        let gst_amount = Number(
          Number(rate) * Number(qty) * (Number(gst_rate) / 100)
        ).toFixed(2);
        let ticked = document.querySelector(
          `input[name="chk_gst${row.item_id}"]`
        );
        ...
        const afterDiscRate =
          Number(temp_items[`txt_rate${row.item_id}`]) - discValue;
    ...
    ////quantity check
      ...
      const disc_percentage = itemDetail[`txt_disc_per${row.item_id}`]
        ? Number(itemDetail[`txt_disc_per${row.item_id}`])
        : Number(temp_items[`txt_disc_per${row.item_id}`]);
      ...
      if (ticked.checked) {
        ...
        //console.log("Hello World444", gst_rate);
        gst_amount = Number(
          Number(afterDiscRate) * Number(value) * (Number(gst_rate) / 100)
        ).toFixed(2);
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <GridContainer className={classes.root}>
                              ...
                              >
                                <div>
                                  <TextField
                                    ...
                                    onChange={(e) => onChange(e, row)}
                                    value={
                                      itemDetail[`txt_disc_value${row.id}`]
                                    }
                                    inputProps={{
                                      style: { textAlign: "right" },
                                    }}
        ...
        >
        {/* {//console.log(location.state.row.dplgrn_no,"26035")} */}
          <StepProceedModel
            step={1}
            isStep={false}
            title="Thank you"
            desc=" Your GRN No is "
            generateNo={`${location.state.row.dplgrn_no}`}
          />
        </MasterModel>
