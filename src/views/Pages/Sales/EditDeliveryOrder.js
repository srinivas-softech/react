...
const DeliveryOrderView = () => {
  ...
  const onChange = (e, row, disp_total_till_now) => {
    const { name, value } = e.target;
    //console.log("rijaji", name,value, locationDetails);
  ...
  const onClickSubmitDeliveryOrder = (e) => {
    ...
    if (allItems.sales_order_item_details.length) {
      allItems.sales_order_item_details.map((item) => {
        ...
        updateItems.push({ ...item, ...item_by_location });
      });
    }
  ...
  React.useEffect(() => {
    ...
    getDeliveryOrderBySalesIdDON(
      ...
      (items) => {
        ...
        setBillDetail((prv) => ({
          ...
          txt_delivery_order_date: dateFormateField(items.DelDate),
          //txt_delivery_order_note: items.delivery_order_note,
          ddl_showroom_warehouse: "",
          // txt_delivery_period_from: dateFormateField(
          //   items.delivery_order_from_date
          // ),
          // txt_delivery_period_to: dateFormateField(
          //   items.delivery_order_to_date
          // ),
          other_dois: items.other_dois,
        }));

        let oldLocDetails = {}
        
        items.delivery_order_item_details.map((item,i) => {
          let item_qty_attrs = Object.keys(item).filter( k => /^\d+_\d+$/.test(k) );
          item_qty_attrs.map((o,i) => oldLocDetails = { ...oldLocDetails, [o]: item[o] });
        });
        //console.log("old loc details", oldLocDetails);
    ...
    //console.log("lccc", locationDetails);
  ...
  //console.log(allItems.delivery_order_item_details, "dui");
                ...
                <Table className={classes.table} aria-label="customized table">
                  ...
                  <TableBody>
                    <StyledTableRow>
                      ...
                      <StyledTableCell align="left" className={classes.endate}>
                        {dateFormate(allItems.sales_order_date)}
                      </StyledTableCell>
